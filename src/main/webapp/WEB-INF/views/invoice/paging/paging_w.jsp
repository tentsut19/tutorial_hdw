...
<div class="row">
	<div class="col-md-5">
		<div class="dataTables_info" id="table-1_info" role="status"
			aria-live="polite">Showing ${(pagination_w.currentPage - 1) * pagination_w.itemPerPage + 1} to 
			<c:if test="${(pagination_w.itemPerPage * pagination_w.currentPage) > pagination_w.totalItem}">${pagination_w.totalItem }</c:if>
			<c:if test="${(pagination_w.itemPerPage * pagination_w.currentPage) < pagination_w.totalItem}">${pagination_w.itemPerPage * pagination_w.currentPage}</c:if> of
		...
		<div class="dataTables_paginate paging_simple_numbers" style="float: right;"
			...
			<ul class="pagination">
				...
				<c:forEach var="i" begin="1" end="${pagination_w.pageSize }">
					...
					<c:if test="${(pagination_w.pageSize) <= 7}">
						<c:if test="${i == pagination_w.currentPage}">
					...
					<c:if test="${(pagination_w.pageSize) > 7}">
						<c:if test="${pagination_w.currentPage > 4 }">
							<!-- สร้าง pagina แรกและ... ไว้ -->
							<c:if test="${i == 1 }">
								<li class="paginate_button page-item <c:if test="${i == pagination_w.currentPage}">active</c:if>">
									<a href="${pageContext.request.contextPath}/${pagination_w.url }/page/${i}/itemPerPage/${pagination_w.itemPerPage}/tab/W"
										aria-controls="table-1" data-dt-idx="1" tabindex="0"
										class="page-link">${i}
									</a>
							...
							<c:if test="${(pagination_w.pageSize-4) >= pagination_w.currentPage }">
								<c:if test="${i == pagination_w.currentPage || i == pagination_w.currentPage-1 || i == pagination_w.currentPage+1 }">
									<li class="paginate_button page-item <c:if test="${i == pagination_w.currentPage}">active</c:if>">
										<a href="${pageContext.request.contextPath}/${pagination_w.url }/page/${i}/itemPerPage/${pagination_w.itemPerPage}/tab/W"
											aria-controls="table-1" data-dt-idx="1" tabindex="0"
											class="page-link">${i}
								...
								<c:if test="${i == pagination_w.pageSize }">
								<li class="paginate_button page-item disabled">
									<a href="" class="page-link">...</a>
								</li>
								<li class="paginate_button page-item <c:if test="${i == pagination_w.currentPage}">active</c:if>">
							...
							<c:if test="${(pagination_w.pageSize-5) < pagination_w.currentPage }">
								<c:if test="${(pagination_w.pageSize-5) < i && pagination_w.currentPage > (pagination_w.pageSize-4)}">
									<li class="paginate_button page-item <c:if test="${i == pagination_w.currentPage}">active</c:if>">
										<a href="${pageContext.request.contextPath}/${pagination_w.url }/page/${i}/itemPerPage/${pagination_w.itemPerPage}/tab/W"
											aria-controls="table-1" data-dt-idx="1" tabindex="0"
											class="page-link">${i}
						...
						<c:if test="${pagination_w.currentPage <= 4 }">
							...
							<c:if test="${(pagination_w.pageSize) == i}">
							<li class="paginate_button page-item <c:if test="${i == pagination_w.currentPage}">active</c:if>">
								<a href="${pageContext.request.contextPath}/${pagination_w.url }/page/${i}/itemPerPage/${pagination_w.itemPerPage}/tab/W"
									aria-controls="table-1" data-dt-idx="1" tabindex="0"
									class="page-link">${i}
